...
1. A rich, 8 sentence opening summary of the day's overarching narrative and market tone, written as TWO paragraphs separated by a line break (output a blank line between them). The first paragraph must be exactly the first two sentences: a scene-setting lede that captures the day's headline and the overall market tone. Then start a new paragraph for the remaining six sentences, which explain the dominant forces at play, characterize investor mood and risk appetite, and weave in the most notable cross-asset or sector moves so it reads like a polished analyst's morning note. Keep it flowing and readable, not a list. Finish with a sentence on the index that led the day and how it compared to the others, for example: The small-cap Russell 2000 index led the day's gainers, surging 2.4%, while the large-cap indexes closed up over 1% across the board.
